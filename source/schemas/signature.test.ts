...
describe("Signature Schema", () => {
  describe("FieldConstraintSchema", () => {
    ...
    it("should reject invalid minLength (negative)", async () => {
      ...
      await expect(Effect.runPromise(Schema.decodeUnknown(FieldConstraintSchema)(invalidConstraints)))
        .rejects.toThrow()
    })
  ...
  describe("InputFieldSchema", () => {
    ...
    it("should reject input field without name", async () => {
      ...
      await expect(Effect.runPromise(validateInputField(invalidInputField)))
        .rejects.toThrow()
    })
  ...
  describe("OutputFieldSchema", () => {
    ...
    it("should reject output field without name", async () => {
      ...
      await expect(Effect.runPromise(validateOutputField(invalidOutputField)))
        .rejects.toThrow()
    })
  ...
  describe("SignatureInstructionsSchema", () => {
    ...
    it("should reject instructions without main", async () => {
      ...
      await expect(Effect.runPromise(validateSignatureInstructions(invalidInstructions)))
        .rejects.toThrow()
    })
  })
  ...
  describe("SignatureMetadataSchema", () => {
    ...
    it("should reject invalid timestamp format", async () => {
      ...
      await expect(Effect.runPromise(validateSignatureMetadata(invalidMetadata)))
        .rejects.toThrow()
    })
  ...
  describe("SignatureSchema", () => {
    ...
    it("should reject signature without input fields", async () => {
      ...
      await expect(Effect.runPromise(validateSignature(invalidSignature)))
        .rejects.toThrow()
    })
  ...
  describe("Factory Functions", () => {
    ...
    it("should reject invalid data in createValidatedSignature", async () => {
      ...
      await expect(Effect.runPromise(createValidatedSignature(invalidInputFields, outputFields, instructions)))
        .rejects.toThrow()
    })
  })
  ...
  describe("Serialization", () => {
    ...
    it("should handle JSON serialization round-trip", () => {
      const signature = {
        ...
        outputFields: [
          createOutputField("answer", "Answer")
        ],
        instructions: {
          main: "Answer the question",
          chainOfThought: true
  ...
  describe("Edge Cases", () => {
    it("should handle signature with many fields", async () => {
      const manyInputFields = Array.from({ length: 50 }, (_, i) => 
        createInputField(`input_${i}`, `Input field ${i}`)
      )
