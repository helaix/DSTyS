...
describe("Example Schema", () => {
  describe("ExampleDataSchema", () => {
    ...
    it("should reject non-object data", async () => {
      ...
      await expect(Effect.runPromise(validateExampleData(invalidData)))
        .rejects.toThrow()
    })
  })
  ...
  describe("ExampleMetadataSchema", () => {
    it("should validate complete metadata", async () => {
      const validMetadata = {
        dspy_uuid: "123e4567-e89b-12d3-a456-426614174000",
        dspy_split: "train",
        source: "dataset_v1",
    ...
    it("should reject invalid timestamp format", async () => {
      ...
      await expect(Effect.runPromise(validateExampleMetadata(invalidMetadata)))
        .rejects.toThrow()
    })
  ...
  describe("ExampleSchema", () => {
    ...
    it("should reject example without data", async () => {
      ...
      await expect(Effect.runPromise(validateExample(invalidExample)))
        .rejects.toThrow()
    })
  ...
  describe("Factory Functions", () => {
    ...
    it("should reject invalid data in createValidatedExample", async () => {
      ...
      await expect(Effect.runPromise(createValidatedExample(invalidData)))
        .rejects.toThrow()
    })
  })
